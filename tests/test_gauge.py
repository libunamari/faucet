...
class GaugePrometheusTests(unittest.TestCase):
    ...
    def parse_prom_output(self, output):
        ...
        for line in output.split('\n'):
            ...
            if not line.startswith(gauge_prom.PROM_PORT_PREFIX):
                continue

            #get the stat name e.g. of_port_rx_bytes and strip 'of_port_'
            prefix = gauge_prom.PROM_PORT_PREFIX + gauge_prom.PROM_PREFIX_DELIM
            stat_name = re.search(r'(.*){', line).group(1)
            stat_name = stat_name.replace(prefix, '')

            dp_id = re.search(r'dp_id="(.+?)"', line).group(1)
            dp_id = int(dp_id, 16)

            port_name = re.search(r'port_name="(.+?)"', line).group(1)
